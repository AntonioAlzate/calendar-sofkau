...
public class SchedulerService {
    ...
    public static final String PROGRAMA_NO_EXISTE = "El programa academnico no existe";

    @Autowired
    private ProgramRepository programRepository;

    // Completado --> TODO: deben retornar un flux de programDate Flux<ProgramDate>
    public Flux<ProgramDate> generateCalendar(String programId, LocalDate startDate) {
        ...
        // Completado -->TODO: debe pasarlo a reactivo, no puede trabaja elementos bloqueantes
        // Completado -->TODO: trabajar el map reactivo y no deben colectar
        var program = programRepository.findById(programId);

        return program
                .flatMapMany(p -> Flux.fromStream(getDurationOf(p)))
                .map(toProgramDate(startDate, endDate, pivot[0], index))
                .switchIfEmpty(Mono.error(new RuntimeException(PROGRAMA_NO_EXISTE)));
    }
